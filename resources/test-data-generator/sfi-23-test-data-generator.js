...
function createSqlForTotals(sbi) {
    ...
    for (let i = 1; i <= NUMBER_OF_TOTALS; i++) {
        var resultString = TOTALS_INSERT_TEMPLATE_SQL.replace('SBI_REPLACE', sbi)
...
function createSqlForDax(calculationId) {
    ...
    for (let i = 1; i <= NUMBER_OF_DAX; i++) {
        ...
        var resultString = DAX_INSERT_TEMPLATE_SQL.replace('CALCULATION_ID_REPLACE', calculationId)
...
function createSqlForDelinkedCalculations(sbi) {    
    for (let i = 1; i <= NUMBER_OF_DC; i++) {
        var resultString = DELINKEDCALSULATION_INSERT_TEMPLATE_SQL.replace('APPLICATION_ID_REPLACE', i)
