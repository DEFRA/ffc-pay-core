...
function parseArgs () {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--dry-run') {
      ...
      console.log('Usage: node tools/pull/pull-pay-submission-data.js [--dry-run] [--force] [--limit N]')
...
async function run () {
  ...
  try {
    ...
    hostedConnection = await createRecoveryConnection({ database: HOSTED_DATABASE, applicationName: 'ffc_pay_submission_data_pull' })

    console.log('Connecting to local recovery database (write)...')
    localConnection = await createLocalConnection({ applicationName: 'ffc_pay_submission_data_pull' })
    ...
    console.log('\nSubmission data pull complete.')
  } catch (error) {
    console.error('Failed to pull submission data:', error.message)
    process.exit(1)
  } finally {
    if (hostedConnection) {
