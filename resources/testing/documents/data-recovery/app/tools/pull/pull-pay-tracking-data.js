...
function parseArgs () {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--dry-run') {
      ...
      console.log('Usage: node tools/pull/pull-pay-tracking-data.js [--dry-run] [--force] [--limit N]')
...
async function getKeysToPull (localConnection, limit) {
  let sql = `SELECT "sourceSystem", frn, "agreementNumber"
     FROM public."${LOCAL_QUEUE_TABLE}"
     WHERE "${PARENT_TABLE.flagColumn}" = true
     ORDER BY "sourceSystem", frn, "agreementNumber"`
...
async function run () {
  ...
  try {
    ...
    hostedConnection = await createRecoveryConnection({ database: HOSTED_DATABASE, applicationName: 'ffc_pay_tracking_data_pull' })

    console.log('Connecting to local recovery database (write)...')
    localConnection = await createLocalConnection({ applicationName: 'ffc_pay_tracking_data_pull' })

    await copyParentTable(hostedConnection, localConnection, options)

    console.log('\nTracking data pull complete.')
  } catch (error) {
    console.error('Failed to pull tracking data:', error.message)
    process.exit(1)
  } finally {
    if (hostedConnection) {
