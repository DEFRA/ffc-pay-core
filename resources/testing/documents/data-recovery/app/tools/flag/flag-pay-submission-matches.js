...
const config = require('../../config/pay-submission')
...
function parseArgs () {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--dry-run') {
      ...
      console.log('Usage: node tools/flag/flag-pay-submission-matches.js [--dry-run]')
      console.log('Flags submission paymentRequests using natural keys from the local pay-processing paymentRequests table.')
      process.exit(0)
    }
  }
...
async function ensureLocalRecoveryDb () {
  try {
    const connection = await createLocalConnection({ applicationName: 'ffc_pay_submission_flag_probe' })
...
async function run () {
  ...
  try {
    ...
    hostedConnection = await createRecoveryConnection({ database: HOSTED_DATABASE, applicationName: 'ffc_pay_submission_flag_reader' })

    console.log('Connecting to local recovery database (write)...')
    localConnection = await createLocalConnection({ applicationName: 'ffc_pay_submission_flag_writer' })
