...
function parseArgs () {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--dry-run') {
      ...
      console.log('Usage: node tools/flag/flag-pay-request-editor-matches.js [--dry-run]')
      console.log('Flags request-editor paymentRequests using natural keys from the local pay-processing paymentRequests table.')
      process.exit(0)
    }
  }
...
async function loadSourceKeysFromLocalPayments (localConnection) {
  ...
  if (rows.length === 0) {
    throw new Error(`No rows found in local public."${SOURCE_TABLE.table}". Run pay-processing recovery first.`)
  }
...
async function findMatchedPaymentRequests (hostedConnection, sourceKeys) {
  ...
  for (let i = 0; i < sourceKeys.length; i += BATCH_SIZE) {
    ...
    const { rows } = await hostedConnection.query(
      `SELECT "agreementNumber", frn, "schemeId"
...
async function markPaymentRequestsFound (localConnection, matchedRows) {
...
async function ensureLocalRecoveryDb () {
  try {
    const connection = await createLocalConnection({ applicationName: 'ffc_pay_request_editor_flag_probe' })
...
async function run () {
  ...
  try {
    ...
    hostedConnection = await createRecoveryConnection({ database: HOSTED_DATABASE, applicationName: 'ffc_pay_request_editor_flag' })

    console.log('Connecting to local recovery database (write)...')
    localConnection = await createLocalConnection({ applicationName: 'ffc_pay_request_editor_flag' })
    ...
    console.log(`\nFinding matching paymentRequests in hosted ${SERVICE_NAME}...`)
    const matchedRows = await findMatchedPaymentRequests(hostedConnection, sourceKeys)
    const uniqueMatchedRows = Array.from(
      new Map(matchedRows.map(row => [`${row.frn}|${row.agreementNumber}|${row.schemeId}`, row])).values()
    )
    console.log(`Found ${matchedRows.length} matching paymentRequests (${uniqueMatchedRows.length} unique business keys)`)

    console.log('\nUpserting matches into local verification queue...')
    const upserted = await upsertMatchedQueue(localConnection, uniqueMatchedRows)
    console.log(`Upserted ${upserted} queue entries`)

    const flagged = await markPaymentRequestsFound(localConnection, uniqueMatchedRows)
    console.log(`Flagged ${flagged} queue entries as foundInPaymentRequests`)

    console.log('\nRequest-editor flagging complete.')
  } catch (error) {
    console.error('Failed to flag request-editor matches:', error.message)
    process.exit(1)
  } finally {
    if (hostedConnection) {
